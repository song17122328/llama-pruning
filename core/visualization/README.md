...
- **智能颜色编码**（基于层目标比例）:
  ...
  - 保留比例图:
    - 绿色：高于层目标保留比例（保留良好）
    - 橙色：低于层目标保留比例（保留不足）
- **完整标注**: 所有柱子都显示精确百分比（保留1位小数）
- **智能标题**:
  - 显示**模型整体比例**（包含embedding和lm_head的整体剪枝/保留比例）
  - 显示**层目标比例**（Transformer层应达到的剪枝/保留目标，由于embedding和lm_head未剪枝，此值高于整体比例）
  - 示例: `各层剪枝比例 (模型整体: 20.0%, 层目标: 23.0%)`
- **简洁网格线系统**:
  - 浅色虚线：20%, 40%, 60%, 80%, 100% 刻度标记
  - **层目标线**：醒目的橙色/绿色虚线，标记Transformer层的目标剪枝/保留比例
...
- 纵轴: 剪枝百分比 (0-100%)
- **颜色含义**:
  - 红色柱子：高于层目标剪枝比例（该层剪枝比目标更激进）
  - 蓝色柱子：低于层目标剪枝比例（该层剪枝比目标更保守）
- **橙色虚线**: Transformer层目标剪枝比例

**保留比例图** (Retention Ratio):
- 横轴: 层索引 (0-31)
- 纵轴: 保留百分比 (0-100%)
- **颜色含义**:
  - 绿色柱子：高于层目标保留比例（该层保留参数良好）
  - 橙色柱子：低于层目标保留比例（该层保留参数不足）
- **绿色虚线**: Transformer层目标保留比例

**关于两个比例的说明**：
- **模型整体比例** = 整个模型（包括embedding、transformer层、lm_head）的平均剪枝/保留比例
- **层目标比例** = Transformer层应达到的剪枝/保留目标
- 由于 embedding 和 lm_head 通常不剪枝，层目标比例会高于整体比例
- 例如：模型整体剪枝20%时，Transformer层目标剪枝23%才能实现整体20%的目标
